...
const TrainTabBar: React.FC = () => {
    ...
    // Determine active train from URL
    const match = location.pathname.match(/\/admin\/train\/(\d+)/);
    ...
    const handleTabClick = (trainNo: string) => {
        ...
        navigate(`/admin/train/${trainNo}`, { state: { fromTab: true } });
    };
    ...
    const handleClose = (e: React.MouseEvent, trainNo: string) => {
        ...
        if (trainNo === activeTrainNo) {
            if (remaining.length > 0) {
                navigate(`/admin/train/${remaining[0].trainNo}`, { state: { fromTab: true } });
            } else {
                navigate('/admin');
            }
        }
